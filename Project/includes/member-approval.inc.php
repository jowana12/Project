...
$update_stat = $dbh->prepare("update project.registration set p_status=? where reg_id=?");
$update_stat->bindParam(1, $upper);
$update_stat->bindParam(2, $id);
$update_stat->execute();

$stat = $dbh->prepare("select * from project.registration where reg_id=?");
$stat->bindParam(1, $id);
$stat->execute();
$row = $stat->fetch();
...
$add_stat = $dbh->prepare("insert into project.officers values('', ?, ?, ?, ?, ?, ?, ?)");
