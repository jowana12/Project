...
if(isset($_POST['submit'])){
	...
	if($fund <= 0){
		...
		if(empty($org) || empty($fund)){
			...
			$organization = $dbh->prepare("select * from project.organizations where organization_name=?");
			...
			$org_stat = $dbh->prepare("update project.organizations set budget=?, checker=? where organization_name=?");
